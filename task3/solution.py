...
def get_unique_seconds(intervals) -> set[int]:
    seconds = set()
    for start, end in list(zip(intervals[::2], intervals[1::2])):
        for second in range(start, end):
            seconds.add(second)
    return seconds


def presence(intervals: dict[str, list[int]]) -> int:
    lesson_seconds = get_unique_seconds(intervals["lesson"])
    pupil_seconds = get_unique_seconds(intervals["pupil"])
    tutor_seconds = get_unique_seconds(intervals["tutor"])
    return len(lesson_seconds & pupil_seconds & tutor_seconds)
...
if __name__ == '__main__':
    for i, test in enumerate(tests):
        test_answer = presence(test['intervals'])
        assert test_answer == \
            test['answer'], \
