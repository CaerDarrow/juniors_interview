...
def appearance(intervals: dict[str, list[int]]) -> int:
    ...
    # Т. е., первый элемент кортежа - таймстэмп, второй - метка учителя (или ученика), метка входа (или выхода)
    ...
    for (timestamp, actor, action) in common:
        # Проход по общему списку таймстэмпов
        match actor:
            case 'pupil':
                # Если таймстэмп ученика
                # Фиксируется его присутствие на уроке
                pupil_in = True if action == IN else False
            case 'tutor':
        ...
        if pupil_in and tutor_in and lesson_in:
            # Если и учитель, и ученик на уроке
            # Фиксирование момента их присутствия
            start = timestamp
            together = True
        elif together:
            # Если кто-то из них вышел с урока, или урок закончился.
            together = False
            # Суммирование накопленного совместного времени пребывания.
            sum_interval += timestamp - start
