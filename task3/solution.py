def appearance(intervals: dict[str, list[int]]) -> int:
    ...
    for i in range(0, len(tutor), 2):
        for j in range(0, len(pupil), 2):
            delta = min(tutor[i + 1], pupil[j + 1], lesson_end) - max(
                tutor[i], pupil[j], lesson_beggining
            )
...
def normalize_intervals(intervals: list[int]) -> list[int]:
    normalized_intervals:list = []
    for i in range(0, len(intervals), 2):
        applied:bool = False
        for j in range(0, len(normalized_intervals), 2):
            if (
                min(normalized_intervals[j + 1], intervals[i + 1])
                - max(normalized_intervals[j], intervals[i])
                > 0
            ):
                applied = True
                if (
                    intervals[i] >= normalized_intervals[j]
                    and intervals[i + 1] >= normalized_intervals[j + 1]
                ):
