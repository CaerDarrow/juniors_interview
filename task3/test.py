...
@pytest.mark.parametrize('intervals, result',
                         [(item['intervals'], item['answer']) for item in data]
                         )
def test_appearance(intervals, result):
    """Сравнение результ работы функции с известным результатом"""
    assert appearance(intervals) == result
